function [Taux Yaux]=ejercicio2_inciso1_inciso2_auxiliar(T , H, C, h0, finalLlenado, paso, multiplicador, graficar)
  %Correción: Decia 3.35 en vez de 3.34.
fun0=@(t,h)(multiplicador*3.34*((t/1000)^5)*exp(-0.667*t/1000))/((27.5*(h.^3))-(372*(h.^2))+2130*h); %%llenado y sin vaciado
fun1=@(t,h)((multiplicador*3.34*((t/1000)^5)*exp(-0.667*t/1000))-C*((h-H).^(3/2)))/((27.5*h.^3)-(372*h.^2)+2130*h);%%llenado y con vaciado
...
title(['Aproximacion del nivel del agua (Multiplicador: ' num2str(multiplicador) ')']);
ylabel('Nivel del agua (m)');
xlabel('Tiempo (segundos)');
textString =['(' num2str(Taux(index)) ',' num2str(max) ')'];
legend({'Aproximacion por Runge-Kutta de orden 4',['Maximo ' textString]},'Location','southeast');
axis tight;
